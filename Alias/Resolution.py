...
def send_parallel_alias_probes(g, l_l_vertices, ttl, destination):
    ...
    for i in range(0, default_alias_icmp_probe_number):
        ...
        for j in range(0, max_len):
            for l_vertices in l_l_vertices:
                # Small optimization to get the most possible probes at a time
                v = l_vertices[j % len(l_vertices)]
                flow_id = ttls_flow_ids[v][ttl][0]
                alias_udp_probe = build_probe(destination, ttl, flow_id)
...
def elimination_stage(g, elimination_stage_candidates, full_alias_candidates, ttl, destination):
    ip_address = g.vertex_properties["ip_address"]
    elimination_to_remove = set()
    if len(elimination_stage_candidates) > 0:
        for k in range(0, default_number_mbt):
            print str(k) + " round of elimination stage..."
...
if __name__ == "__main__" :
    time_serie1 = [[1521814985.067271, 1521814985.533613, 48497], [1521814986.506872, 1521814986.656282, 48501], [1521814988.167634, 1521814988.464445, 48505], [1521814990.350884, 1521814991.447821, 48509], [1521814993.673899, 1521814993.97282, 48513], [1521814994.979467, 1521814995.295948, 48517], [1521814997.730009, 1521814998.067947, 48521]]



    time_serie2 = [[1521814984.920404, 1521814985.026637, 48496], [1521814986.279711, 1521814986.455487, 48500], [1521814987.770706, 1521814988.128119, 48504], [1521814989.648104, 1521814990.304262, 48508], [1521814993.252151, 1521814993.629116, 48512], [1521814994.771374, 1521814994.927314, 48516], [1521814996.709325, 1521814997.668831, 48520]]
